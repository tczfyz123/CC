...
%>
  ...
  <form name="form1" id="form1" method="post" action="">
   搜索: 编号：<input name="bianhao" type="text" id="bianhao" /> 名称：<input name="mingcheng" type="text" id="mingcheng" />课程：<select name='kecheng' id='kecheng'><option value="">所有</option><% sql="select kechengmingcheng from kechengxinxi order by id desc";RS_result=connDbBean.executeQuery(sql);while(RS_result.next()){%><option value="<%= RS_result.getString("kechengmingcheng")%>" ><%=RS_result.getString("kechengmingcheng")%></option><%}%></select> 主讲人：<input name="zhujiangren" type="text" id="zhujiangren" />
   <input type="submit" name="Submit" value="查找" />
</form>
<table width="100%" border="1" align="center" cellpadding="3" cellspacing="1" bordercolor="00FFFF" style="border-collapse:collapse">  
  <tr>
    <td width="30" align="center" bgcolor="CCFFFF">序号</td>
    <td bgcolor='#CCFFFF'>编号</td><td bgcolor='#CCFFFF'>名称</td><td bgcolor='#CCFFFF'>课程</td><td bgcolor='#CCFFFF'>主讲人</td><td bgcolor='#CCFFFF'>文件</td>
...
%>
  <tr>
    ...
    <td><%=bianhao %></td><td><%=mingcheng %></td><td><%=kecheng %></td><td><%=zhujiangren %></td><td><a href='<%=wenjian %>' target='_blank'>下载</a></td>
    <td width="138" align="center"><%=addtime %></td>
    <td width="90" align="center"><a href="xuexishipin_updt.jsp?id=<%=id%>">修改</a>  <a href="del.jsp?id=<%=id %>&tablename=xuexishipin" onclick="return confirm('真的要删除？')">删除</a> <a href="xuexishipin_detail.jsp?id=<%=id%>" target="_blank">详细</a></td>
  </tr>
  	<%
  }
   %>
...
<p align="center">&nbsp;共<%=zgs%>条记录&nbsp;&nbsp;<%=page_record %>条/页　<a href="xuexishipin_list.jsp?page=1">首页</a>　<a href="xuexishipin_list.jsp?page=<%= curpage-1%>">上一页</a>　<A href="xuexishipin_list.jsp?page=<%= curpage+1%>">下一页</A>　<a href="xuexishipin_list.jsp?page=<%=zys %>">尾页</A>　当前第<FONT color=red><%=curpage %></FONT>页/共<FONT color=red><%=zys %></FONT>页</p>
  </body>
</html>
